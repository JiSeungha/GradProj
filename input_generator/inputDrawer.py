...
def render(t, gGlobalOffset):
    ...
    for i in range(bone_number):
        glColor3ub(0, 0, 255)
        if(i == 0):
            glColor3ub(255, 255, 255)
            current_coordinate = [gGlobalOffset[t][i][0],gGlobalOffset[t][i][1],gGlobalOffset[t][i][2]]
        else:
            current_coordinate = [gGlobalOffset[t][i][0] + gGlobalOffset[t][0][0],gGlobalOffset[t][i][1] + gGlobalOffset[t][0][1],gGlobalOffset[t][i][2] + gGlobalOffset[t][0][2]]
...
def main():
    global frame_number
    path_dir = str(os.getcwd()) + '/Export/withoutRoot/result'
    offsets   = fetchInput(path_dir + '/test_Input.txt')

    if not glfw.init():
        return

    window = glfw.create_window(800, 800, "input", None, None)
    if not window:
        glfw.terminate()
        return
